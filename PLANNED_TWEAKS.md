...
- LiveRoleplayStore is utter shit and needs to use SemaphoreSlim, ConcurrentDictionary, DistributedCache, just be a proper implementation.
...
- When duplicating a story, we need to make sure images from the original story are "linked" to the current story. This means that a story should "remember" the source story with linked `ChatId` array field of stories that share resources. This would allow recursive copies.

- The sidebar needs to be collapsable and we need reactive mobile layout. This would ideally be entirely JavaScript based instead of relying on Blazor.

## Story Cards
- "Story premise" should be like the TV series premise that is consistent through all seasons and episodes
- Story cards should be either a season-scope (long running) or an episode-scope (short running)

Example
- Story premise is "Star Trek Voyager, trapped in the delta quadrant on their own"
- Season Story Card: Year of hell, they're trapped in a massive void of space without access to planetary bodies for resources, everyone is stircrazy and rationing adding pressure
- Episode Story Card: Critical equipment malfunction. Because of the compounding story and season, this story is going to be about salvaging other components to get this critical piece going again. When it might otherwise be about trying to make a trade or a salvaging wreck from an abandoned ship without the season-level impact.

- Role cards need character onboarding section to inject memories
...
- User facing name for credits is "Ink"
...
Icons: single `fa-droplet`, collection `bottle-droplet`
